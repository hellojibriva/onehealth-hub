...
export default function CollectModal({ open, onClose }: Props) {
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    if (!isOnline) {
      saveToQueue(form); setQueueLen(getQueue().length);
    ...
    try {
      ...
      if (existingLoc) {
        ...
        const { data: newLoc } = await supabase.from('locations').insert({
          name: form.location_name, state: form.state, lga: form.lga || null,
          geopolitical_zone: '',
      ...
      await supabase.from('outbreaks').insert({
        ...
        severity: form.severity, status: 'ACTIVE', location_id: locationId,
        start_date: form.start_date, description: form.description || null,
    ...
    for (const item of queue) {
      try {
        ...
        if (existingLoc) {
          ...
          const { data: newLoc } = await supabase.from('locations').insert({
            name: item.location_name, state: item.state, lga: item.lga || null,
            geopolitical_zone: '',
        ...
        await supabase.from('outbreaks').insert({
          ...
          severity: item.severity, status: 'ACTIVE', location_id: locationId,
          start_date: item.start_date, description: item.description || null,
  ...
  return (
    <div
      style={{
        ...
        backgroundColor: 'rgba(0,0,0,0.6)', padding: '0 1rem',
      }}
    >
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-gray-800 dark:text-gray-200">Report Outbreak</h2>
          </div>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1">Disease / Pathogen *</label>
              <input value={form.disease_name} onChange={e => update('disease_name', e.target.value)}
            ...
            <div>
              ...
              <input value={form.reported_by} onChange={e => update('reported_by', e.target.value)}
            ...
            <div className="col-span-2">
              ...
              <textarea value={form.description} onChange={e => update('description', e.target.value)}
                placeholder="Additional details about the outbreak..." rows={3}
          ...
          <button onClick={handleSubmit} disabled={saving || saved}
            className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium transition-colors ${
              saved ? 'bg-teal-600 text-white' : 'bg-teal-600 hover:bg-teal-700 text-white disabled:opacity-60'
            }`}>
            <Send size={14} />
